function bgswitchigratingv4(numreps,duration,pixelsPerPeriod,xcen,ycen,centersize,mask,mtype,tetris,scale,rcenter,rectsize,selector,cort,gray)
...
tiltInDegrees = [0 45 90 135];
topval = 255;
for q = 1:length(tiltInDegrees)
    tiltInRadians(q) = tiltInDegrees(q) * pi / 180; % The tilt of the grating in radians.
end
...
for ii = 1:length(tiltInDegrees)
    ...
    a=cos(tiltInRadians(ii))*radiansPerPixel;
    b=sin(tiltInRadians(ii))*radiansPerPixel;
    xphase = xcen-512;
    yphase = xcen-384;
    ...
    grating1(ii)= Screen('MakeTexture', w, grayscaleImageMatrix);
    grating2(ii)= Screen('MakeTexture', w, grayscaleImageMatrix2);
end
for i = 1:numreps
for t = 1:length(tiltInDegrees)
    for k = 1:(dur*2)
        if(mod(k,2) == 0)
